...
/**
 * Method for get tile's densities as image buffer, which will be updated to 
 * current tile's number of valid samples.
 * @return Image Returns tiles buffer as image.
 */ 
Image *TileManager::GetDensityBuffer()
{
	// Check if density buffer exists
	if(this->densityBuffer == NULL) {
		this->densityBuffer = new Image(this->resolution.x(), this->resolution.y());
	}
	
	
	// Get max density for 'simple proportion' to maximize max variance (to 255Y color)
	//float maxDens = 0.0f;
	//for(unsigned i = 0; i < this->tilesList.Size(); i++) {
	//	maxDens = Max(maxDens, this->tilesList[i]->GetValidSamplesNum());
	//}
	...
	for(unsigned i = 0; i < this->tilesList.Size(); i++) {
		Tile *tile = this->tilesList[i];
		Vector2 offset = tile->GetOffset();
		//float density = tile->GetValidSamplesNum() / maxDens;
		//Color clr = (this->pixelsPerTile > tile->GetValidSamplesNum()) ? Color(density, density, 0.0f) : Color(0.0f, density, density);
		float density = tile->GetValidSamplesNum() / (this->pixelsPerTile);
		Color clr = (density > 1.0f) ? Color::White() : Color(density, density, 0.0f);
		this->densityBuffer->Fill(offset.x(), offset.y(), this->tileSize.x(), this->tileSize.y(), clr);
	}
 ...
 */ 
TileManager *TileManager::SetSamplingMethod(const string &m) 
{ 
	// Sets to all tiles
	for(unsigned i = 0; i < this->tilesList.Size(); i++) {
		this->tilesList[i]->SetSamplingMethod(m);
	}
 ...
 * @return Provides fluent interface.
 */ 
TileManager *TileManager::SetSamplingMethod(const unsigned &m)
{
	// Sets to all tiles
	for(unsigned i = 0; i < this->tilesList.Size(); i++) {
		this->tilesList[i]->SetSamplingMethod(m);
...
/**
 * Sets sample cache size to all tiles. Size determines how much samples will
 * precompute and stored in sampleCache array.
 * @param unsigned s New sample cache size.
 * @return Provides fluent interface.
 */ 
TileManager	*TileManager::SetSampleCacheSize(const unsigned &s)
{
	// Sets to all tiles
	for(unsigned i = 0; i < this->tilesList.Size(); i++) {
		this->tilesList[i]->SetSampleCacheSize(s);
	}
