...
{
	int samplesCnt = 0;
	unsigned minPixels = this->context->GetRedrawFixedPixels();
	unsigned timeSpan = (unsigned)(this->scene->GetAnimationManager()->GetTimeStep() * 1000.0f);
	...
	// Simulate clock ticks a go to next frame
	UpdateSimulatedClock(timeSpan);
